        ...
        <q-item-label>
          <q-input dense outlined label="输入作者名称" @keyup.enter="handleAuthors" v-model="result.form.name">
            <template v-slot:append>
              <q-icon :name="icon.matSearch" style="cursor: pointer;" @click="handleAuthors" />
            </template>
          </q-input>
        </q-item-label>
      </q-item-section>
...
<script setup>
 ...
 const handleQuery = (query) => {
     app.$router.push({path: "/authors", query: {...form.value, ...query}})
 }

 const handleRemoveQuery = (key) => {
     let query = {...form.value}
     delete query[key]
     app.$router.push({path: "/authors", query: query})
 }
